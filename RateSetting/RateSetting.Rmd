...
+ Stop guessing setpoints and stop scanning for setpoints
...
+ Errors-in-variables 
    ...
    + It _tends_ to increase apparant precision of estimates.
...
+ *Peak Data* :Simple regression of monthly peaks on exogenouse variables, i.e., just restrict the data.
...
+ Last seen in an IRP
+ Most common problem is that it assumes that the error terms are symetric.
    + Remember that the peak load is not just extream values for exogenous variables
...
+ Prefered tool but has not hit the PUC level yet.
+ Idea
...
+ You can litterally say, given known weather, we expect the load to be below x, 90% of the time.
